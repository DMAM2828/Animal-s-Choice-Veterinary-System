...
if(isset($_GET['id'])){
    ...
    $qry = $conn->query("SELECT a.*, c.name AS pet_type FROM `appointment_list` a INNER JOIN category_list c ON a.category_id = c.id WHERE a.id = '$appointment_id'");
    if($qry) {
        if($qry->num_rows > 0){
            $res = $qry->fetch_array();
            foreach($res as $k => $v){
            ...
            $service_ids = $res['service_ids']; // Assuming service_ids is the correct column name
            $service = "N/A"; // Default value if service_ids is NULL or empty
            if (!is_null($service_ids) && !empty($service_ids)) {
        ...
        // Handle query error
        echo "Error fetching appointment details: " . $conn->error;
    }
} else {
    echo "<script>alert('Appointment Request ID is required'); location.replace('./?page=appointments');</script>";
}

$doctor_name = isset($doctor_name) ? $doctor_name : '';
if (isset($doctor_id)) {
    $doctor_query = $conn->query("SELECT CONCAT(firstname, ' ', lastname) as doctor_name FROM `users` WHERE id = $doctor_id");
    
    // Check if the query was successful
    if ($doctor_query) {
        $doctor_result = $doctor_query->fetch_assoc();
        if ($doctor_result) {
                                    ...
                                    <fieldset>
                                        ...
                                        <table class="table table-stripped table-bordered" data-placeholder='true' id="">
                                            ...
                                            <tbody>
                                                ...
                                                <tr class="border-info">
                                                <th class="py-1 px-2 text-light bg-gradient-info">Doctor</th>
                                                <td class="py-1 px-2 text-right"><?= $doctor_name ?></td>
                                                </tr>
                                                <tr class="border-info">
                                                <th class="py-1 px-2 text-light bg-gradient-info">Appointment Time</th>
                                                <td class="py-1 px-2 text-right"><?= ($appointment_time) ?>
</td>
                                               </tr>
                                            </tbody>
                                        </table>
